...
def encodedocx(cover, payload, lsb):
...
def decodedocx(stegoFile, lsb):
    ...
    for i in range(0, len(stegoString)):
        stegoByteList.append(dataToBinary(stegoString[i]))
    ...
    # Write the decoded data back to a file and save it in the current directory
    current_dir = os.getcwd()  # Get the current working directory
